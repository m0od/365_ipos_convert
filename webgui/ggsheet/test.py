import json
from datetime import datetime

import gspread
from oauth2client.service_account import ServiceAccountCredentials
from gspread_formatting import *

JSON_CRED = {
    "type": "service_account",
    "project_id": "kt365-387014",
    "private_key_id": "9eb19ad641f9d39c1088f931f3c43954e68b2367",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "kt365service@kt365-387014.iam.gserviceaccount.com",
    "client_id": "109188131360468428010",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/kt365service%40kt365-387014.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
}
SCOPE = [
    'https://www.googleapis.com/auth/spreadsheets',
    'https://www.googleapis.com/auth/drive'
]

class Sheet(object):
    def __init__(self):
        self.cred = ServiceAccountCredentials.from_json_keyfile_dict(JSON_CRED, SCOPE)
        self.wb = None
        self.sheet_id = None
        self.sheet_url = None

    def create_sheet(self, domain=None, title=None):
        client = gspread.authorize(self.cred)
        print('LOGGED')
        title = f"{domain}_{title}_{datetime.now().strftime('%d-%m-%y %H:%M')}"
        self.wb = client.open_by_key('1a3CLjpp3kXZDuLaiAYLBxs0sKUYOuDAeMw7m94PK6r4')
        print('CREATED')

        self.sheet_id = self.wb.id
        self.sheet_url = self.wb.url
        print(self.sheet_url)

    def insert(self, data=None):
        ws = self.wb.get_worksheet(0)
        print(ws.row_values(1))
        f = open('data.json', 'r')
        s = f.read().strip()
        f.close()
        js = json.loads(s)
        values= []
        print(list(js[0].keys()))
        for _ in js:
            v = [
                _.get('Code'), _.get('Name'), _.get('BranchName'), _.get('NumberOfOrder'),
                _.get('Revenue'), _.get('VAT'), _.get('Refund'), _.get('NetRevenue'),
                _.get('ATM'), _.get('BAEMIN'), _.get('CA_THE'),_.get('CARD'), _.get('CASH'),
                _.get('CHUYEN_KHOAN'),_.get('CREDIT_CARD'), _.get('EPAYMENT'), _.get('GOJEK'),_.get('GRABFOOD'),
                _.get('LANDLORDCASH'),_.get('LANDLORDCRCARD'),_.get('MASTERCARD'),_.get('MERCHANDISE_EXCHANGE_VOUCHER'),
                _.get('MOMO'),_.get('PARAFAIT_CARD'), _.get('PAYOO'),_.get('PAYOO_POS'), _.get('PAYOOPOS'),
                _.get('QUET_THE'), _.get('SHOPEEFOOD'), _.get('THANH_TOAN_THE'), _.get('THE'), _.get('THE_ATM'),
                _.get('THE_NGAN_HANG'), _.get('THE_NGAN_HANG_QR_PAYOO'), _.get('TIEN_THE'), _.get('TK_NGAN_HANG'), _.get('TK_THE'),
                _.get('VISA'), _.get('VNPAY'), _.get('VNPAYPOS'), _.get('VNPAYQR'),
                _.get('VOU_GOTIT'), _.get('VOUCHER'), _.get('ZALOPAY')
                 ]
            # ['Mã cửa hàng', 'Tên cửa hàng', 'Chi nhánh', 'Hoá đơn', 'Doanh thu', 'VAT', 'Trả hàng', 'Doanh thu (NET)']
            # ['Code',        'Name',  'BranchName', 'NumberOfOrder', 'Revenue', 'VAT',   'Refund', 'NetRevenue', 'RetailerId', 'BranchId'
            # a = ['ATM', 'CA_THE', 'CASH', 'CHUYEN_KHOAN', 'CREDIT_CARD', 'GOJEK', 'GRABFOOD', 'LANDLORDCASH',
            #  'LANDLORDCRCARD', 'MERCHANDISE_EXCHANGE_VOUCHER', 'PARAFAIT_CARD', 'PAYOO', 'PAYOO_POS', 'PAYOOPOS',
            #  'QUET_THE', 'SHOPEEFOOD', 'THANH_TOAN_THE', 'THE', 'THE_NGAN_HANG', 'THE_NGAN_HANG_QR_PAYOO',
            #  'TK_NGAN_HANG', 'TK_THE', 'VISA', 'VNPAY', 'VNPAYPOS', 'VNPAYQR', 'VOUCHER']
            values.append(v)
        # print(values[0])
        # values = [
        #     ['Mã hàng hóa', 'Tên hàng hóa', 'Tên nhóm', 'ĐVT', 'ĐVT Lớn', 'Mã ĐVT Lớn', 'Giá trị quy đổi', 'Giá bán',
        #      'Giá bán ĐVT Lớn', 'Giá vốn', 'Tồn kho', 'Thuộc tính', 'Hình ảnh', 'Ghi chú nhanh khi bán hàng',
        #      'Định mức tồn nhỏ nhất', 'Định mức tồn lớn nhất', 'Loại hàng', 'Không cho phép bán',
        #      'Tách thành nhiều dòng khi bán hàng', 'Tên máy in', 'Mã nhà cung cấp', 'VAT', 'Thành phần',
        #      'Quản lý theo Lô Hạn', 'Không in ra tem nhãn', 'Mở Extra/Topping khi chọn', 'In nhiều vị trí', 'Hoa hồng',
        #      'Id']]
        # values.extend(data)
        ws.insert_rows(values, row=2)
        # set_frozen(ws, 1)
        # fmt = CellFormat(
        #     backgroundColor=Color(147 / 255, 204 / 255, 234 / 255),  # set it to yellow
        #     textFormat=TextFormat(bold=True),
        # )
        # format_cell_range(ws, '1:1', fmt)

a = Sheet()
a.create_sheet()
a.insert()
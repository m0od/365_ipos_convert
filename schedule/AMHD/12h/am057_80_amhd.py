...
class AM057(object):
    ...
    def get_cpn(self):
        ...
        DATA = self.scan_file(f'*M*xlsx')
        if not DATA:
            submit_error(retailer=self.ADAPTER_RETAILER, reason='CPN NOT_FOUND')
            return
        ws = openpyxl.load_workbook(DATA, data_only=True)
        raw = ws['Orders']
        code = None
        orders = {}
        for row in range(2, raw.max_row + 1):
            stt = raw[row][0].value
            if stt:
                code = raw[row][1].value
                pur_date = raw[row][3].value
                pur_date = datetime.strptime(pur_date, '%Y-%m-%d')
                pur_date = pur_date.strftime('%Y-%m-%d 07:00:00')
                total = float(raw[row][6].value)
                discount = float(raw[row][5].value)
                p_name = raw[row][7].value.upper()
                orders.update({
                    code: {

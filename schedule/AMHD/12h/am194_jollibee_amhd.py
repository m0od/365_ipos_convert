...
class AM194(object):
    def __init__(self):
        ...
        self.jollibee_email = 'jbvn158@gmail.com'
    ...
    def get_attachments(self):
        try:
            ...
            search = []
            search += ['FROM', self.jollibee_email]
    ...
    def get_time_data(self):
        ...
        files = self.scan_file('*time*.xls')
        # print(DATA)
        # DATA = self.scan_file('*time.xls')
        if not len(files):
    ...
    def get_sys_data(self):
        from pos_api.adapter import submit_order, submit_error, submit_payment
        # DATA = self.scan_file('system*report.xls')
        files = self.scan_file('*.xls')
        if not len(files):
            submit_error(self.ADAPTER_RETAILER, 'TIME_NOT_FOUND')
            return
        for DATA in files:
                ...
                for k in keyword:
                    if data.startswith(k):
                        ...
                        if k == 'BUSINESS DATE':
                            ...
                            for _ in range(i, nRows):
                                data = str(raw.row(i)[0].value).upper()
                                # print(data)
                            ...
                            # print(row, str(raw.row(i)[0].value).upper())
                            try:
    ...
    def get_data(self):
        ...
        for item in self.ATTACHMENTS:
            # print(251, item.get_filename().lower())
